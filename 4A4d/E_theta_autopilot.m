...
%%
kinner = 0.9;
inner = lag_e*E_theta*s;
sys = feedback(-inner, kinner);

rlocus(-inner)
hold on 
r = rlocus(-inner, kinner);
...
step(sys/s, 100)
hold on
step(feedback(K*sys/s, 1), 100)
stepinfo(feedback(K*sys/s, 1))
% plot_c_star(0.95)
%
plot_long_rlocus(sys/s,K)

figure
impulse(-E_theta, 100)
hold on
impulse(feedback(K*sys/s, 1), 100)
